...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel("dash-" + userId)
      .on("postgres_changes",
        { event: "*", schema: "public", table: "tasks", filter: `user_id=eq.${userId}` }, () => load())
      .on("postgres_changes",
        { event: "*", schema: "public", table: "agents", filter: `user_id=eq.${userId}` }, () => load())
      .subscribe();
  ...
  const kpis = [
    { label: "Active Agents", value: String(stats.running), sub: `${stats.agents} total`, icon: Bot },
    { label: "Tasks Today", value: String(stats.today), sub: "since midnight", icon: ListChecks },
    { label: "Success Rate", value: successRate, sub: `${stats.completed}/${stats.total}`, icon: CheckCircle2 },
    { label: "Status", value: stats.running > 0 ? "Live" : "Idle", sub: "fleet status", icon: Activity },
  ];
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight" style={{ letterSpacing: "-0.02em" }}>Dashboard</h1>
      ...
      <div className="grid gap-4 mb-6" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>
        {kpis.map((k) => {
          const Icon = k.icon;
          return (
            <div key={k.label} className="rounded-lg p-5"
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <div className="rounded-lg p-10 text-center"
          ...
          <p className="text-[13px]" style={{ color: "var(--color-text-muted)" }}>
            Create agents in <strong>Agents</strong>, then queue work in <strong>Tasks</strong>. AI runs are powered by Lovable AI Gateway.
          </p>
        </div>
        <LiveFeed />
      </div>
